...
sql = "select produceTime ,tomcat_request_count from request_time where id<120"
cursor.execute(sql)
result=cursor.fetchall()
deletesql="delete from predict_time_source"
cursor.execute(deletesql)
deletesql1="delete from predict_time"
...
df['tomcat_request_count']=df['tomcat_request_count'].astype('float32')
...
pd.io.sql.to_sql(df, 'predict_time_source', yconnect, schema='bigdata', if_exists='append')

# 将原来的dataframe格式的数据转变为Series类型，并且为其添加时间索引。
ts = pd.Series(df['tomcat_request_count'].values)
...
dict_pre = {'produceTime':pre.index,'tomcat_request_count':pre.values,'type':1}
...
yconnect = create_engine('mysql+pymysql://root:123456@localhost:3306/bigdata?charset=utf8')
pd.io.sql.to_sql(df_pre, 'predict_time', yconnect, schema='bigdata', if_exists='append' )
