...
sql = "select produceTime ,system_cpu_pct_usage from system_cpu where id<120"
cursor.execute(sql)
result=cursor.fetchall()
deletesql="delete from predict_cpu_source"
cursor.execute(deletesql)
deletesql1="delete from predict_cpu"
...
df['system_cpu_pct_usage']=df['system_cpu_pct_usage'].astype('float32')
...
pd.io.sql.to_sql(df, 'predict_cpu_source', yconnect, schema='bigdata', if_exists='append')

# 将原来的dataframe格式的数据转变为Series类型，并且为其添加时间索引。
ts = pd.Series(df['system_cpu_pct_usage'].values)
...
dict_pre = {'produceTime':pre.index,'system_cpu_pct_usage':pre.values,'type':1}
...
yconnect = create_engine('mysql+pymysql://root:123456@localhost:3306/bigdata?charset=utf8')
pd.io.sql.to_sql(df_pre, 'predict_cpu', yconnect, schema='bigdata', if_exists='append' )
